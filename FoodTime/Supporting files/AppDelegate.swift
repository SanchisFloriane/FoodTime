...
class AppDelegate: UIResponder, UIApplicationDelegate, GIDSignInDelegate {
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
        ...
        Auth.auth().signInAndRetrieveData(with: credential, completion: { (authResult, error) in
            ...
            let user = User(lastname: user.profile.familyName, firstname: user.profile.name, pseudo: user.profile.givenName, email: user.profile.email, profilePictureFIRUrl: user.profile.imageURL(withDimension: 200).absoluteString, fbAccount: false.description, googleAccount: true.description)
